...
packages depend on ``led-controller-version-3==<this version>``, not ``>=``, because they are
...
def test_every_optional_distribution_is_reachable_through_an_extra():
    """Nothing publishable may be unreachable through led-controller-version-3.

    A distribution that is built and uploaded but that no extra installs is a
    package nobody can get to except by knowing it exists. led-controller-version-3 is not a
    metapackage any more — it carries the runtime itself — so what has to hold
    is about the extras, not about its dependencies.
    """
    extras = manifest("led-controller-version-3")["project"]["optional-dependencies"]
    named = {
        PIN.match(item.strip())["name"]
        for name, group in extras.items() if name != "all"
        for item in group
    }
    assert named == INTERNAL - {"led-controller-version-3"}
...
def test_the_runtime_distribution_declares_no_internal_dependency():
    ...
    """
    declared = manifest("led-controller-version-3")["project"]["dependencies"]
