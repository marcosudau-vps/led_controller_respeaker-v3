...
def test_everything_is_derived_from_the_one_root(tmp_path):
    ...
    assert project.build_target("core-set") == (
        tmp_path / "packages/ledctrl-v3-set-core/src/lefx/sets/core_set/core-set.lefxset"
    )
...
def test_every_entry_point_distribution_travels_with_the_bundle():
    ...
    # The hardware's entry points now live in ledctrl-v3 itself, together with
    ...
    assert {"ledctrl-v3", "ledctrl-v3-device-simulated-respeaker"} <= registered
