...
``.lefxset`` could be. Now each set has exactly one home — ``src/lefx/sets/
<module>/`` inside its own distribution — and that home is the same directory
in a checkout, in an editable workspace and in an installed wheel. A set that is
built is a set that is installed; there is no step in between to forget.

Which distribution directory that is gets looked up rather than computed, so
that renaming a distribution does not silently move where builds land. The
constant below is only the fallback for a project that has no package for the
set yet, which is a project where somebody is about to add one.
"""

NEW_SET_DISTRIBUTION = "led-controller-version-3-set-{short}"
...
class Project:
    ...
    def set_module_dir(self, set_name: str) -> Path:
        """The ``src/lefx/sets/<module>`` directory that ships the named set.

        Found by looking for the module rather than by assembling a
        distribution name: the module path is fixed by the import name and the
        distribution name is not. When no distribution has it yet — a project
        where the set is new — the conventional name is used, and creating the
        directory is what makes it real.
        """
        module = set_name.replace("-", "_")
        packages = self.root / SET_PACKAGE_DIR
        for candidate in sorted(packages.glob(f"*/src/lefx/sets/{module}")):
            if candidate.is_dir():
                return candidate
        short = set_name.removesuffix("-set") or set_name
        return (
            packages / NEW_SET_DISTRIBUTION.format(short=short)
            / "src" / "lefx" / "sets" / module
        )

    def build_target(self, set_name: str) -> Path:
        """Where the named set's archive belongs, built or not."""
        return self.set_module_dir(set_name) / f"{set_name}.lefxset"
    ...
    def built_set_dirs(self) -> list[Path]:
        ...
        """
        found = self.root.glob(f"{SET_PACKAGE_DIR}/*/src/lefx/sets/*")
